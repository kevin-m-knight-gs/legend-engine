...
|---|---|
| 1 | Create the `legend-engine-core-emit` parent aggregator with the `legend-engine-emit` core module and `legend-engine-emit-junit` JUnit binding module. Implement the core data model (`EMITResult`, `EMITPhase`, `EMITPhaseResult`), `emit.yaml` parsing (`EMITModelDescriptor`), and file loading (`EMITModelLoader`). |
...
| 3 | Implement `EMITTestSuiteBuilder` and `EMITModelDiscovery` in `legend-engine-emit-junit` for JUnit integration and granular task discovery. |
