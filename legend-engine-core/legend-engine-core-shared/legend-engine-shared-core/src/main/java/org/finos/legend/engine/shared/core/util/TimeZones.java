 ...
 *
 * <p><b>This class is a copy, retained temporarily.</b> It mirrors
 * {@code org.finos.legend.pure.m4.tools.time.TimeZones}, which legend-pure has but has not yet
 * released. It exists so that the callers here can move to one way of resolving a time zone name
 * without waiting for that release. When a legend-pure carrying that class is available, delete
 * this one and point its callers at that one; the two have the same methods and answer alike, so
 * nothing but the import changes.
 */
